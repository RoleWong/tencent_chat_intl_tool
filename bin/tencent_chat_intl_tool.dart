...
import 'package:tencent_chat_intl_tool/common/utils.dart';
...
void main(List<String> arguments) async {
  ...
  while (!isPathValid) {
    ...
    try {
      packagePath =
          pubspec['dependency_overrides']['tencent_chat_i18n_tool']['path'];
    } catch (_) {}
    try {
    ...
    if (packagePath != null) {
      ...
      if (usePath == null || usePath.toLowerCase() == 'y') {
        ...
        stdout.write(
            "Please enter the local path to your forked repository for further customization: ");
      ...
      stdout.write(
          "If you do not specify a local path in 'pubspec.yaml', subsequent operations will not take effect.\n");
      stdout.write(
          "Please enter the local path to your forked repository for further customization: ");
      packagePath = stdin.readLineSync();
      if (packagePath != null && packagePath.trim().isNotEmpty) {
  ...
  stdout.write(
      "A. Add new language entries for internationalization. This action must be performed before using new entries.\n");
  stdout.write(
      "B. Retrieve the tool's built-in language entries, including arb files for all supported languages, and save them to your project directory for further customization.\n");
  stdout.write(
      "C. Inject your modified language arb files for all languages back into the tool, allowing the updated configuration to take effect.\n");
...
void readNewEntriesFromFile(String packagePath) async {
  ...
  Directory(p.join(packagePath, 'lib', 'language_arb'))
      .listSync()
...
void addLanguageToBase(String packagePath) async {
  ...
  final result = await Process.run('flutter', ['gen-l10n'],
      workingDirectory: packagePath, runInShell: true);

  if (result.exitCode == 0) {
    stdout.write("\nSuccessfully generated localization configs.\n");
  } else {
    stdout.write(
        "Failed to generate localization files. Please manually run 'flutter gen-l10n' in the tencent_chat_intl_tool package directory.\n");
  }
}
